# ০০ - স্টক মার্কেটের রোডম্যাপ (Stock Market Roadmap)

অনেক নতুন ট্রেডার সরাসরি ক্রয়/বিক্রয় (Buy/Sell) শিখতে গিয়ে ক্ষতিগ্রস্ত হয়। আসলে মার্কেটের শেখার একটি নির্দিষ্ট ধাপ আছে। নিচের লার্নিং সাইকেল অনুসরণ করলে ভিত্তি অনেক শক্ত হবে।

***বি.দ্রো: এই তথ্য বাংলাদেশ পুঁজিবাজারের আদলে তৈরি করা হচ্ছে***

---

## পর্যায় ১: স্টক মার্কেটের মৌলিক বিষয় (Stock Market Basics)

প্রথমে বুঝতে হবে মার্কেট কীভাবে কাজ করে।

***শিখুন:***

* স্টক মার্কেট কী
* শেয়ার কী
* DSE ও CSE কী
* IPO কী
* Market Cap
* Sector ও Industry
* Bid, Ask, Spread
* Volume ও Turnover

***লক্ষ্য:***

মার্কেটের ভাষা বুঝতে পারা।

---

## পর্যায় ২: ক্যান্ডেলস্টিক মাস্টারী (Candlestick Mastery)

ক্যান্ডেলস্টিক হলো ট্রেডারের ভাষা।

***শিখুন:***

* বুল্লিশ ক্যান্ডেল (Bullish Candle)
* বেয়ারিশ ক্যান্ডেল (Bearish Candle)
* ডোজি (Doji)
* হ্যামার (Hammer)
* শুটিং ষ্টার (Shooting Star)
* এঙ্গুলফিং প্যাটার্ন (Engulfing Pattern)
* মর্নিং ষ্টার (Morning Star)
* ইভনিং ষ্টার (Evening Star)

***বুঝুন***

```text
Open
  │
  ▼
 ┌───┐
 │   │ ← Body
 └───┘
  ▲
  │
Close
```

***লক্ষ্য:***

একটি Candle কী গল্প বলছে তা বুঝতে পারা।

---

## পর্যায় ৩: মার্কেট স্ট্রাকচার (Market structure)

এটাই স্মার্ট মানি কনসেপ্ট (Smart Money Concept - SMC) এর ভিত্তি।

***শিখুন:***

* আপট্রেন্ড (Uptrend)
* ডাউনট্রেন্ড (Downtrend)
* রেঞ্জ (Range)
* হায়ার হাই (Higher High - HH)
* হায়ার লো (Higher Low - HL)
* লোয়ার হাই (Lower High - LH)
* লোয়ার লো (Lower Low - LL)

### আপট্রেন্ড (Uptrend)

```text
HH
    ▲
   / \
  /   \ HH
 /     ▲
HL    HL
```

***লক্ষ্য:***

ট্রেন্ড (Trend) সনাক্ত করতে পারা।

---

## পর্যায় ৪: সাপোর্ট ও রেজিস্টেন্স (Support & Resistance)

***শিখুন:***

* সাপোর্ট (Support)
* রেজিস্টেন্স (Resistance)
* ব্রেকআউট (Breakout)
* রিটেস্ট (Retest)

***লক্ষ্য:***

কোথায় Buy/Sell Pressure আছে বুঝতে পারা।

---

## পর্যায় ৫: সাপ্লাই ও ডিমান্ড (Supply & Demand)

***শিখুন:***

* ডিমান্ড জোন (Demand Zone)
* সাপ্লাই জোন (Supply Zone)
* ফ্রেশ জোন (Fresh Zone)
* টেস্টেড জোন (Tested Zone)

***লক্ষ্য:***

বড় ক্রেতা ও বিক্রেতাদের এলাকা খুঁজে বের করা।

---

## পর্যায় ৬: লিকুইডিটি কনসেপ্ট (Liquidity Concepts)

এখন স্মার্ট মানি (Smart Money) এর জগতে প্রবেশ।

***শিখুন:***

* লিকুইডিটি (Liquidity)
* লিকুইডিটি সুইপ (Liquidity Sweep)
* বাই সাইড লিকুইডিটি (Buy Side Liquidity)
* সেল সাইড লিকুইডিটি (Sell Side Liquidity)
* ইকুয়াল হাই (Equal High)
* ইকুয়াল লো (Equal Low)

***লক্ষ্য:***

রিটেল ট্রেডের (Retail Trader) কোথায় ফাঁদ (Trap) হতে পারে তা বুঝতে পারা।

---

## পর্যায় ৭: স্মার্ট মানি কনসেপ্ট (Smart Money Concepts - SMC)

***শিখুন:***

* অর্ডার ব্লক - Order Block (OB)
* ব্রেকার ব্লক - Breaker Block (BB)
* ফেয়ার ভ্যালু গ্যাপ - Fair Value Gap (FVG)
* মার্কেট স্ট্রাকচার শিফট - Market Structure Shift (MSS)
* চেঞ্জ অফ ক্যারেক্টার - Change of Character (CHOCH)
* ব্রেক অফ স্ট্রাকচার - Break of Structure (BOS)

***শেখার ক্রম:***

```text
Liquidity Sweep
        ↓
CHOCH / MSS
        ↓
Order Block
        ↓
Entry
```

***লক্ষ্য:***

ইনস্টিটিউশনাল ফুটপ্রিন্ট (Institutional Footprint) খুঁজে পাওয়া।

---

## পর্যায় ৮: টেকনিক্যাল ইন্ডিকেটর (Technical Indicators)

অনেকেই এখান থেকে শুরু করে, কিন্তু এটি পরে শেখা উচিত।

***শিখুন:***

* মুভিং অ্যাভারেজ - Moving Average (MA)
* ই.এম.এ - EMA
* আর.এস.আই - RSI
* এম.এ.ছি.ডি - MACD
* ভি.ডাবলু.এ.পি - VWAP
* ভলিউম প্রোফাইল - Volume Profile

***লক্ষ্য:***

প্রাইস অ্যাকশন (Price Action - PA) নিশ্চিত করা।

---

## পর্যায় ৯: রিস্ক ম্যানেজমেন্ট (Risk Management)

সবচেয়ে গুরুত্বপূর্ণ অধ্যায় যেখানে নতুন ট্রেডারদের ৯০% এখানেই ব্যর্থ হয়।

***শিখুন:***

* রিস্ক/রিওয়ার্ড - Risk/Reward Ratio
* পজিশন সাইজিং - Position Sizing
* স্টপ লস - Stop Loss
* সর্বোচ্চ ড্রডাউন - Maximum Drawdown
* মূলধন সংরক্ষণ - Capital Preservation

***গোল্ডেন রুল (Golden Rule):***

```text
Risk : Reward = 1 : 2
বা
Risk : Reward = 1 : 3
```

***লক্ষ্য:***

লোকসান (Loss) ছোট রাখা।

---

## পর্যায় ১০: মার্কেট সাইকোলজি (Market Psychology)

একজন পেশাদার ট্রেডার এবং একজন অপেশাদার ট্রেডারের মধ্যে সবচেয়ে বড় পার্থক্যটি এই মার্কেটের সাইকোলজি মাধ্যমেই তৈরি হয়।

***শিখুন:***

* ভয় - Fear
* লোভ - Greed
* কিছু হারানোর ভয় - FOMO
* রেভেঞ্জ মার্কেটের - Revenge Trading
* শৃঙ্খলা - Discipline
* ধৈর্য - Patience

***পেশাদার মানসিকতা (Professional Mindset):***

✅ সেটআপ না থাকলে ব্যবসা না করা।

✅ ক্ষতি মেনে নেয়া।

✅ প্ল্যান ফলো করুন।

❌ ওভার মার্কেটের না করা।

❌ ইমোশনাল মার্কেটের না করা।

---

## পর্যায় ১১: বিল্ড ওয়ান স্ট্রাটেজি (Build One Strategy)

একসাথে ১০টি স্ট্রাটেজি (Strategy) শিখবেন না।

একটি স্ট্রাটেজি বেছে নিন।

***উদাহরণ:***

```text
Market Structure
      +
Liquidity Sweep
      +
Order Block
      +
Risk Management
```

***লক্ষ্য:***

একই সেটআপ (Setup) বারবার কার্যকর (Execute) করা।

---

## পর্যায় ১২: ব্যাকটেস্টিং (Backtesting)

***কমপক্ষে:***

* ১০০-২০০ ট্রেড রিভিউ।
* উইন রেট বের করা।
* রিস্ক/রিওয়ার্ড পরিমাণ করা।

***জার্নাল রাখুন:***

```text
ডেট - Date
স্টক - Stock
এন্ট্রি - Entry
স্টপ লস - Stop Loss
টার্গেট - Target
ফলাফল - Result
ভুল - Mistake
```

---

## পর্যায় ১৩: ডেমো (Demo)

***সঠিক পদ্ধতি:***

```text
শেখা
  ↓
ব্যাকটেস্ট (Backtest)
  ↓
ডেমো ট্রেড (Demo Trade)
  ↓
ক্ষুদ্র মূলধন (Small Capital)
  ↓
সংগতি (Consistency)
  ↓
স্কেল আপ (Scale Up)
```

***ভুল পদ্ধতি:***

```text
শেখা → ১ লাখ টাকা বিনিয়োগ করা
```

---

## সারসংক্ষেপ: প্রফেশনাল মার্কেটের লার্নিং সাইকেল - Professional Trading Learning Cycle

```text
মার্কেট ব্যাসিক্স - Market Basics
      ↓
ক্যান্ডেলস্টিক - Candlestick
      ↓
মার্কেট স্ট্রাকচার - Market Structure
      ↓
সাপোর্ট/রেজিস্টেন্স - Support/Resistance
      ↓
সাপ্লাই/ডিমান্ড - Supply & Demand
      ↓
লিকুইডিটি - Liquidity
      ↓
এসএমসি কনসেপ্টস - SMC Concepts
      ↓
ঝুঁকি ব্যবস্থাপনা - Risk Management
      ↓
সাইকোলজি - Psychology
      ↓
কৌশল তৈরি করুন - Build Strategy
      ↓
ব্যাকটেস্টিং - Backtesting
      ↓
লাইভ মার্কেটের - Live Trading
      ↓
জার্নাল পর্যালোচনা - Journal Review
      ↓
ইম্প্রোভ - Improve
      ↓
রিপিট - Repeat
```

**পেশাদার ট্রেডার হওয়ার লক্ষ্য থাকলে, শেখাকে অগ্রাধিকার দেওয়া উচিত:**

`Market Structure → Liquidity → SMC → Risk Management → Psychology`

কারণ বেশিরভাগ লাভজনক ট্রেডাররা ইন্ডিকেটরের চেয়ে প্রাইস অ্যাকশন, লিকুইডিটি এবং রিস্ক ম্যানেজমেন্টকে বেশি গুরুত্ব দেন।

| পরের ধাপ | আগের ধাপ |
| --------- | ---------- |
| [০১ - স্টক মার্কেটের মৌলিক বিষয় (Stock Market Fundamentals)](../০১-স্টক-মার্কেটের-মৌলিক-বিষয়) | [স্টক মার্কেট (Stock Market)](../) |

> [!TIP]
>
> **🙏 অনুরোধ:** কোনো ভুল পেলে আপনি এই বিষয়বস্তুটি সংশোধন করতে পারেন। আপনার অবদান এই প্রজেক্টকে আরও সমৃদ্ধ করবে। ধন্যবাদ।
